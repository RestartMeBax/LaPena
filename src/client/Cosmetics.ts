...
export async function purchaseCosmetic(
  ...
  if (resolved.type === "pattern") {
    const patternKey = colorPaletteName
      ...
      : `pattern:${c.name}`;
    settings.setSelectedPatternName(patternKey);
  } else if (resolved.type === "flag") {
    settings.setFlag(`flag:${c.name}`);
  }
...
function hasOwnedPatternFlare(
  ...
  const baseFlare = `pattern:${patternName}`;
  if (flares.includes(baseFlare)) {
    return true;
  }

  if (colorPaletteName) {
    const exactFlare = `pattern:${patternName}:${colorPaletteName}`;
    if (flares.includes(exactFlare)) {
      return true;
    }
  }

  return flares.some((flare) => flare.startsWith(`${baseFlare}:`));
}
...
export function patternRelationship(
  ...
  if (colorPalette.isArchived) {
    ...
    if (hasOwnedPatternFlare(flares, pattern.name, colorPalette.name)) {
      return "owned";
    }
    return "blocked";
  }
