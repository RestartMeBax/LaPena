...
export class LeaderboardPlayerList extends LitElement {
  @property({ type: String }) mode: "players" | "mostWins" = "players";
  ...
  private lastMode: "players" | "mostWins" = "players";
  ...
  private visiblePlayers(): PlayerLeaderboardEntry[] {
    if (this.mode !== "mostWins") {
      return this.playerData;
    }
    return [...this.playerData].sort((a, b) => {
  ...
  private renderPlayerRow(player: PlayerLeaderboardEntry, index: number) {
    const isCurrentUser = this.currentUserEntry?.playerId === player.playerId;
    const displayRank = this.mode === "mostWins" ? index + 1 : player.rank;
    ...
    return html`
      ...
      >
          ...
          <div class="flex items-center gap-2">
            ${player.flag
              ? html`<img
                  src=${player.flag}
                  alt=""
                  class="w-5 h-5 rounded-sm object-cover shrink-0 border border-white/10"
                />`
              : ""}
            ${player.clanTag
              ? html`<div
        ...
        ${this.mode === "mostWins"
          ? html`<td class="py-3 px-4 text-right pr-6">
              <span class="font-mono text-yellow-300 font-bold"
                >${player.wins}</span
              >
            </td>`
          : html`
  ...
  render() {
    ...
    const rows = this.visiblePlayers();
            ...
            <table class="w-full text-sm border-collapse table-fixed">
              ...
              <thead class="sticky top-0 z-10">
                ...
                >
                  ...
                  ${this.mode === "mostWins"
                    ? html`<th class="py-4 px-4 text-right font-bold pr-6">
                        WINS
                      </th>`
                    : html`
              ...
              <tbody>
                ${rows.map((player, index) => this.renderPlayerRow(player, index))}
              </tbody>
            </table>
