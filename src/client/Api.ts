...
import {
  ...
  MostWinLeaderboardResponse,
  MostWinLeaderboardResponseSchema,
  NewsItemSchema,
  PlayerProfile,
  PlayerProfileSchema,
...
let __publicAdminMaps: Promise<PublicAdminMap[]> | null = null;
let __publicAdminMapsFetchedAt = 0;
const PUBLIC_ADMIN_MAPS_CACHE_TTL_MS = 10_000;
...
export async function deleteInventoryCosmetic(
  cosmeticType: "pattern" | "skin" | "flag",
  cosmeticName: string,
  colorPaletteName?: string,
): Promise<boolean> {
  try {
    const response = await fetch(`${getApiBase()}/shop/inventory/delete`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: await getAuthHeader(),
    ...
    if (!response.ok) {
      console.error(
        "deleteInventoryCosmetic: request failed",
        response.status,
        response.statusText,
      );
      return false;
    ...
    console.error("deleteInventoryCosmetic: request failed", e);
    return false;
  }
}
...
export async function fetchMostWinLeaderboard(
  page: number,
): Promise<MostWinLeaderboardResponse | "reached_limit" | false> {
  try {
    const url = new URL(`${getApiBase()}/leaderboard/most-win`);
    url.searchParams.set("page", String(page));
    const res = await fetch(url.toString(), {
      headers: { Accept: "application/json" },
    });
    ...
    if (!res.ok) {
      console.warn(
        "fetchMostWinLeaderboard: unexpected status",
        res.status,
        res.statusText,
      );
      return false;
    ...
    const json = await res.json();
    const parsed = MostWinLeaderboardResponseSchema.safeParse(json);
    if (!parsed.success) {
      console.warn(
        "fetchMostWinLeaderboard: Zod validation failed",
        parsed.error.toString(),
      );
      return false;
    }
    ...
    console.error("fetchMostWinLeaderboard: request failed", err);
    return false;
  }
}

export async function getPublicAdminMaps(): Promise<PublicAdminMap[]> {
  const now = Date.now();
  if (
    __publicAdminMaps !== null &&
    now - __publicAdminMapsFetchedAt < PUBLIC_ADMIN_MAPS_CACHE_TTL_MS
  ) {
    return __publicAdminMaps;
  }

  __publicAdminMaps = (async () => {
    try {
      const res = await fetch("/api/admin/public/maps", {
        headers: { Accept: "application/json" },
      });
      if (!res.ok) return [];
      const body = (await res.json()) as { maps?: PublicAdminMap[] };
      if (!Array.isArray(body.maps)) return [];
      return body.maps;
    } catch {
      return [];
    } finally {
      __publicAdminMapsFetchedAt = Date.now();
    }
  })();
