...
export class TerritoryLayer implements Layer {
  ...
  private lastTextureRevisionByPlayer = new Map<number, number>();
  private lastTextureRevisionCheck = 0;
  private hasAnyImageSkins = false;
  ...
  redraw() {
    ...
    const skinContext = this.skinCanvas.getContext("2d");
    if (skinContext === null) throw new Error("2d context not supported");
    this.skinContext = skinContext;
    this.skinCanvas.width = this.game.width();
    this.skinCanvas.height = this.game.height();
    this.skinImageData = this.skinContext.getImageData(
      0,
      0,
      this.skinCanvas.width,
      this.skinCanvas.height,
    );
    ...
    this.borderImageData = this.borderContext.getImageData(
      0,
      0,
    ...
    this.skinContext.putImageData(this.skinImageData, 0, 0);
    this.borderContext.putImageData(this.borderImageData, 0, 0);
  ...
  initImageData() {
    this.game.forEachTile((tile) => {
      ...
      this.skinImageData.data[offset + 3] = 0;
      this.alternativeImageData.data[offset + 3] = 0;
      this.borderImageData.data[offset + 3] = 0;
    });
  }

  renderLayer(context: CanvasRenderingContext2D) {
    const now = Date.now();
    this.enqueueVisibleOwnedTilesForTextureUpdates(now);
    ...
    if (
      ...
      if (w > 0 && h > 0) {
        ...
        if (this.hasAnyImageSkins && !this.alternativeView) {
          this.skinContext.putImageData(this.skinImageData, 0, 0, vx0, vy0, w, h);
        }
        this.borderContext.putImageData(
  ...
  paintTerritory(tile: TileRef, isBorder: boolean = false) {
    ...
    if (this.game.isBorder(tile)) {
      ...
      this.clearBorderTile(tile);
      const territoryColor = owner.territoryColor(tile);
      if (owner.hasImageSkin()) {
        this.paintTile(this.imageData, tile, territoryColor, 0);
        this.paintTile(this.skinImageData, tile, territoryColor, 255);
      } else {
        this.clearSkinTile(tile);
  ...
  clearTile(tile: TileRef) {
    ...
    this.skinImageData.data[offset + 3] = 0; // Set alpha to 0 (fully transparent)
    this.alternativeImageData.data[offset + 3] = 0; // Set alpha to 0 (fully transparent)
    this.borderImageData.data[offset + 3] = 0; // Set alpha to 0 (fully transparent)
  }
  ...
  clearBorderTile(tile: TileRef) {
    const offset = tile * 4;
    this.borderImageData.data[offset + 3] = 0; // Set alpha to 0 (fully transparent)
  }

  clearSkinTile(tile: TileRef) {
    const offset = tile * 4;
    this.skinImageData.data[offset + 3] = 0; // Set alpha to 0 (fully transparent)
  }
  ...
  private enqueueVisibleOwnedTilesForTextureUpdates(now: number) {
    if (now < this.lastTextureRevisionCheck + 100) {
      return;
    }
    this.lastTextureRevisionCheck = now;

    let hasAnyImageSkins = false;
    let changed = false;
    for (const player of this.game.playerViews()) {
      if (player.hasImageSkin()) {
        hasAnyImageSkins = true;
      }
      const key = Number(player.id());
      const revision = player.textureRevision();
      const previous = this.lastTextureRevisionByPlayer.get(key);
      if (previous !== revision) {
        this.lastTextureRevisionByPlayer.set(key, revision);
        changed = true;
      }
    }
    this.hasAnyImageSkins = hasAnyImageSkins;
    ...
    const x1 = Math.min(this.game.width() - 1, bottomRight.x);
    const y1 = Math.min(this.game.height() - 1, bottomRight.y);
