...
export class PlayerView {
  ...
  private imgW = 0;
  private imgH = 0;
  private skinTextureRevision = 0;
  private skinProjectionCache?: {
    tilesOwned: number;
    nameX: number;
    nameY: number;
    centerX: number;
  ...
  constructor(
    ...
    // Load image texture if the cosmetic has an imageUrl
    const imageUrl = cosmetics.pattern?.imageUrl;
    if (imageUrl) {
      const img = new Image();
      ...
      img.onload = () => {
        try {
          const texW = Math.max(
            PlayerView.SKIN_TEXTURE_MIN_SIZE,
            Math.min(PlayerView.SKIN_TEXTURE_MAX_SIZE, img.width),
          );
          const texH = Math.max(
            PlayerView.SKIN_TEXTURE_MIN_SIZE,
            Math.min(PlayerView.SKIN_TEXTURE_MAX_SIZE, img.height),
          );
          ...
          const id = ctx.getImageData(0, 0, texW, texH);
          this.imagePixels = id.data;
          this.imgW = texW;
          this.imgH = texH;
      ...
      img.onerror = () => {
        console.warn(
          "Failed to load skin image; falling back to pattern/solid color",
          imageUrl,
        );
      };
      img.src = imageUrl;
  ...
  textureRevision(): number {
    return this.skinTextureRevision;
  }

  hasImageSkin(): boolean {
    return Boolean(this.cosmetics.pattern?.imageUrl);
  }
  ...
  private skinProjection(): {
    ...
    const nameX = this.nameData.x;
    const nameY = this.nameData.y;

    if (
      this.skinProjectionCache &&
      this.skinProjectionCache.tilesOwned === tilesOwned &&
      this.skinProjectionCache.nameX === nameX &&
      this.skinProjectionCache.nameY === nameY
    ) {
      return this.skinProjectionCache;
    }

    const territorySpan = Math.max(
      6,
      Math.sqrt(Math.max(1, tilesOwned)) * PlayerView.SKIN_TERRITORY_SPAN_PADDING,
    );
    const projection = {
      tilesOwned,
      nameX,
    ...
    this.skinProjectionCache = projection;
    return projection;
  }

  private sampleSkinColor(worldX: number, worldY: number): Colord | null {
    if (!this.imagePixels || this.imgW <= 0 || this.imgH <= 0) {
      return null;
    }
    ...
    const bilinear = (sx: number, sy: number) => {
      ...
      const x1 = Math.min(this.imgW - 1, x0 + 1);
      const y1 = Math.min(this.imgH - 1, y0 + 1);
      const tx = sx - x0;
      const ty = sy - y0;

      const c00 = (y0 * this.imgW + x0) * 4;
      const c10 = (y0 * this.imgW + x1) * 4;
      const c01 = (y1 * this.imgW + x0) * 4;
      const c11 = (y1 * this.imgW + x1) * 4;

      const r0 = lerp(p[c00], p[c10], tx);
      const r1 = lerp(p[c01], p[c11], tx);
      const g0 = lerp(p[c00 + 1], p[c10 + 1], tx);
      const g1 = lerp(p[c01 + 1], p[c11 + 1], tx);
      const b0 = lerp(p[c00 + 2], p[c10 + 2], tx);
      const b1 = lerp(p[c01 + 2], p[c11 + 2], tx);
