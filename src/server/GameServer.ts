...
export class GameServer {
  ...
  private archiveGame() {
    ...
    if (this.onPlayersWon && this.winner?.winner) {
      const winnerClientIds = this.winner.winner.slice(1);
      const winners = [...new Set(winnerClientIds)]
        .map((clientID) => this.allClients.get(clientID))
