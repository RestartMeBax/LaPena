...
export class CosmeticButton extends LitElement {
  ...
  private renderPreview(): TemplateResult {
    if (this.resolved.type === "pattern") {
      ...
      const patternUrl = c ? (c as Pattern).url : undefined;
      if (patternUrl) {
        return html`<img
          src=${patternUrl}
          alt=${c!.name}
          ...
          @error=${(e: Event) => {
            ...
            img.parentElement
              ?.querySelectorAll(".pattern-fallback")
              .forEach((el) => ((el as HTMLElement).style.display = ""));
          }}
        /><span class="pattern-fallback" style="display:none;width:100%;height:100%;">${renderPatternPreview(playerPattern, 150, 150)}</span>`;
      }
