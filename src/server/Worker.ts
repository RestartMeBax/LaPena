...
export async function startWorker() {
  ...
  const gm = new GameManager(config, log, (winners) => {
    for (const winner of winners) {
      authDb.addWin(winner.persistentID, 1, {
        username: winner.username,
        clanTag: winner.clanTag,
        flag: winner.flag,
      });
    }
  });
  ...
  wss.on("connection", (ws: WebSocket, req) => {
    ws.on("message", async (message: string) => {
      ...
      try {
        ...
        let cosmeticResult = privilegeRefresher
          .get()
          .isAllowed(flares ?? [], clientMsg.cosmetics ?? {});

        if (
          cosmeticResult.type === "forbidden" &&
          cosmeticResult.reason.includes("invalid pattern: Pattern") &&
          cosmeticResult.reason.includes("not found")
        ) {
          try {
            await privilegeRefresher.refreshNow();
            cosmeticResult = privilegeRefresher
              .get()
              .isAllowed(flares ?? [], clientMsg.cosmetics ?? {});
          } catch {
            // Keep original result if refresh fails.
          }
        }
