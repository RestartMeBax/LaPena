...
function updateAccountNavButton(userMeResponse: UserMeResponse | false) {
  ...
  const setAccountText = (text: string, title?: string) => {
    if (!signInTextEl) return;
    signInTextEl.textContent = text;
    signInTextEl.title = title ?? text;
  };
  ...
  const showAvatar = (src: string, label: string, alt?: string) => {
    if (avatarEl) {
  ...
  if (discord && avatarEl) {
    ...
    if (url) {
      showAvatar(url, discord.username, avatarAlt);
      return;
    }
  }
